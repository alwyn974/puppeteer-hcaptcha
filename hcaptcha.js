...
const { rdn, mm } = require("./src/utils");
...
const getAnswersTF = async (request_image, tasks) => {
    ...
    for (const task of tasks) {
        threads.push(client.objectLocalization(task.datapoint_uri));
    }
 ...
 * @returns null
 */
const hcaptcha = async (page, visionClient) => {
 ...
 * @returns hCaptcha solved token
 */
const hcaptchaToken = async (url, visionClient) => {
    // Set client passed in to Google Client
    if (!visionClient) {
        return undefined;
    }
